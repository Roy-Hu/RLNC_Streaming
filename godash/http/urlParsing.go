...
// getHTTPClient:
func GetHTTPClient(quicBool bool, debugFile string, debugLog bool, useTestbedBool bool) (*http.Transport, *http.Client, *h2quic.RoundTripper) {
	...
	if quicBool {
		...
		if !useTestbedBool {
			...
			trQuic = &h2quic.RoundTripper{TLSClientConfig: quicConfig}
			// set up the client
...
func GetURL(requrl string, isByteRangeMPD bool, startRange int, endRange int, quicBool bool, debugFile string, debugLog bool, useTestbedBool bool) ([]byte, time.Duration, string) {
	...
	if quicBool {
		...
		bytes, rtt, _ := xnc.Client(fileName, false)
